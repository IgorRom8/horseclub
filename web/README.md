...
Фронтенд на **Next.js**: главная, услуги, инфраструктура, тренеры, конюшни, контакты и внутренние страницы. Форма заявки и публичные настройки (город, мессенджеры, карта) задаются через переменные окружения.
...
- **Prisma 6** + **PostgreSQL** — опционально (часть контента из БД; без БД — из кода)  
